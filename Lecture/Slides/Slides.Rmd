...
- **Networks** that posses a particular property
  - Which are otherwise random
  - e.g. fixed parameter: degree distribution

- **Analysis** performed by comparing (two) different model network structures
  - Example:
    - scale-free networks (degree distribution: power law)
      - vs.
    - non scale-free networks (degree distribution: some other law)

```{r, echo=FALSE, out.width=900, fig.align='center'}
knitr::include_graphics("Figures/scale_free_network.svg")
```

```{r, echo=FALSE, out.width=900, fig.align='center'}
knitr::include_graphics("Figures/non_scale_free_network.svg")
```
...
- Show a number of network structure pairs and ask for the feel of behavior

# F.A.Q.

- Power law

- Dynamic prosesses on a network
...
- For such **Random graph G(n,m)** created in the following method:
  ...
  - Prevent multiedge and self-edge
    - Distinct pairin
    - Only to already not connected verices

```{r, echo=FALSE, out.width=900, fig.align='center'}
knitr::include_graphics("Figures/random_graph.svg")
```

- There is not one but an ensamble of networks that satisfy the fixed constraints

- Probability of such networks $P(G)$:
  - $P(G)=1|\omega$ , for such networks where **omega** is the number of such networks
...
- Show an example of a fixed n,m random network vs. ensamble of other networks
...
- Such **typical networks** can be analysed analitically
...
- Probability distribution of edges
  - Erdos - Renyi
  - Poisson random network graph
  - Bernoulli random network graph


```{r, echo=FALSE, out.width=900, fig.align='center'}
knitr::include_graphics("Figures/erdos_renyi_graph.svg")
```

```{r, echo=FALSE, out.width=900, fig.align='center'}
knitr::include_graphics("Figures/poison_graph.svg")
```

```{r, echo=FALSE, out.width=900, fig.align='center'}
knitr::include_graphics("Figures/bernoulli_graph.svg")
```
...
- Give example of different probability distributions of G(n,p) and ask for behavior traits

---

# Analytical behavior analysis of G(n,p) ensamble - $n$ fixed, $m$ not-fixed

- Mean number of edges $\langle m \rangle$ (expected)
  - What is the number of graphs with exactly (n,m) between **distinct vertex pairs** $\binom{n}{2}$
    - Every graph has the same (unifrom) probability $P(G)$:

$$P(G) = p^m(1-p)^(\frac{n}{2})-m$$

  - What is the probability of drawing a graph (selecting on random) with exactly $m$ edges from the ensamble G(n,p)?

$$P(m) = \binom{\binom{n}{2}}{m}p^m(1-p)^(\frac{n}{2})-m$$

    - this is a **binomial distribution**, so it gets easy to solve

$$\langle m \rangle = \sum_{m=0}^{\binom{n}{2}}mP(m) = \binom{n}{2} p$$

    - distinct vertex pairs * probability of an edge

- Mean degree of vertex $$\langle k \rangle$$ a.k.a. $c$
  - Mean degree $$\langle k \rangle = \frac{2m}{n}$$, where $n$ is a fixed constant

$$\langle k \rangle = \sum_{m=0}^{\binom{n}{2}} \frac{2m}{n} P(m) = \frac{2}{n}\binom{n}{2}p = (n-1)p$$

    - number of other vertices * probability of an edge
    - $\langle k \rangle == c$
...
- Degree distribution $p_{k}$ (for a vertex)
  - What is the probability of a vertex connected to $k$ other vertices (and not with others)?
    - $p^{k}(1-p)^{n-1-k}$ , with $\binom{n-1}{k}$ distinct vertex pairs

$$p_{k} = \binom{n-1}{k}p^{k}(1-p)^{n-1-k}$$

    - this is again a **binomial distribution**, so it gets easy to solve

---

# Excercise 4

- Show examples of these behaviors analytics on a network
...
- Mean degree $c$
  - constant - number of friends does not depend on the number of people in the world
  1 . $c = (n-1)p \Rightarrow p = \frac{c}{n-1}$ , $n\rightarrow\infty$
    - Vanishingly small p

- Degree distribution (for a vertex) $p_k$

$$ln[(1-p)^{n-1-k}] = (n-1-k)ln(1-\frac{c}{n-1}) \simeq -(n-1-k)\frac{c}{n-1} \simeq -c

  - Expand the logarithm as Tayler series
  - Becmoes more exact as $n\rightarrow\infty$
  - $(1-p)^{n-1-k} = e^{-c}$
  - Taking the exponent of both sides, $n\rightarrow\infty$

$$\binom{n-1}{k} = \frac{(n-1)!}{(n-1-k)!k!} \simeq \frac{(n-1)^{k}}{k!}, $n\rightarrow\infty$

$$p_{k} = \binom{n-1}{k}p^k(1-p)^{n-1-k}$$

  - Poissons (degree) distribution for G(n,p) when $n\rightarrow\infty$

$$p_{k} = \frac{(n-1)^k}{k!}p^k e^{-c} = \frac{(n-1)^k}{k!} (\frac{c}{n-1})^k e^{-c} = e^{-c} \frac{c^{k}}{k!}$$ , $n\rightarrow\infty$
...
- Show Poisson distribution + large networks
...
- Clusering coefficient $C$
  ...
  - What is the probability that two vertices are connected (neighbours)?
...
- Poisson random graph: special case of G(n,p) ensamble for large networks $n\rightarrow\infty$
  - Clusering coefficient $C \simeq 0 , n\rightarrow\infty$, if mean degree $c$ is fixed
...
- Show Poisson RNG $n\rightarrow\infty$ and real-world networks and ask for difference - social network

---

# Poisson random graph: special case of G(n,p) ensamble for large networks $n\rightarrow\infty$

- Property: largest component (qualitative difference)
  - $p = 0$ ; independent of size $n$
    - Disconnected network
    - $n$ separate components - vertices isolated
  - $p = 1$ ; dependent of size $n$
    - 1-single component
...
- Behavior: Sensitivity of parameter $p[0,1]$
  ...
  - Size of giant component
    - Let $u$ be fraction of vertices not connected to the giant component
      - $u = 1$ , if there is no giant component
      - $u < 1$ , if there is giant component

---

# Analytical behavor analysis of G(n,p) ensamble for large networks $n\rightarrow\infty$

- What is the probability that the vertex is not connected to the giant component through any of the $n-1$ other vertices in the newtork?
  1. Probability that a vertex $v_{i}$ is not connected to other vertex $v_{j}$

$$1 - p$$

  2. Probability that a vertex $v_{i}$ is connected to other vertex $v_{j}$, but $v_{j}$ is not connected to giant component

$$pu$$ , probability of an edge * probability that the $v_{j}$ doesn't belong to giant component

    - This should be recursive for all other vetices, apart from $v_{i}$
    - For $n\rightarrow\infty$, this doesn't change much from $n-1$ and $n-2$, excuding $v_{i}$ and $v_{j}$ respectivelly
  - Total probability: $1 - p + pu$

$$u = (1-p+pu)^{n-1} = [1 - \frac{c}{n-1}(1-u)]^{n-1}$$

$$ ln u = (n-1)ln[1-\frac{c}{n-1}(1-u)] \simeq -(n-1)\frac{c}{n-1}(1-u) = c(1-u); $n\rightarrow\infty$

$$u = e^{-c(1-u)}$$ ; fraction of vertices not in giant component

$$S = 1-u = 1 - e^{-cS}$$ ; fraction of vertices in giant component

  - Doesn't have simple solution for $S$ in closed form, but possible to plot
  - e.g., for plotting: $c = - \frac{ln(1-S)}{S}$, using Lambert W-function

- Slide Fig. 12.1 complete
  1. Solution $S=0$ ; small c - no giant component
  2. Solution $S=0 and S>0$ ; large c - there is giant component
  - Transition phase happendes in between sna;;/large c, where gradient of the diagonal matches the gradient of the curve for $S=0$

$$\frac{d}{dS}(1-e^{-cS})=1$$

$$ce^{-cS}=1$$ ; setting $S=0$ - pahse transition point $c=1$

  - therefore RNG can have a giant component only if $c>1$
  - if $c\leq1$ than $S=0$ and there is no giant component
...
# Final problem: What is the size of the giant component?

- Already proven: for $c\leq1$ than $S=0$ and there is no giant component - $S=0$
- Need to prove: for $c>1$ there is giant component
  - Two solutions $S=0 and S>0$, which one is true?
  - Proof: Fig 12.2
