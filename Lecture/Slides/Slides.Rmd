...
.pull-left[
- There is not one but an ensamble of networks that satisfy the fixed constraints, e.g., n=20 and m=22
...
- Mean number of edges $\langle m \rangle$
  - **Distinct vertex pairs** $\binom{n}{2}$ * probability of an edge $p$; e.g. 190 combinations
...
- Degree distribution $p_{k}$ (for a vertex with $k$ others)
...
- Mean degree $c$
  - constant over large networks - number of friends does not depend on the number of people in the world
